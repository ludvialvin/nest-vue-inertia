  ...
  <div>
    <template v-if="profile">
      <div class="flex flex-col gap-1">
        <h2 class="text-2xl font-bold tracking-tight text-slate-900">{{ profile.name }}</h2>
        <p class="text-sm text-slate-500">Profil user dengan dynamic route /users/:id.</p>
      </div>
    ...
    <div v-else class="rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
      <p class="text-4xl font-bold text-brand-200">?</p>
      <h2 class="mt-2 text-xl font-bold text-slate-900">User Tidak Ditemukan</h2>
      <p class="mt-1 text-sm text-slate-500">Data user yang anda cari tidak tersedia.</p>
    </div>
